...
let cantidadCanciones = (banda1.albums);

let cantidadTotalCanciones = 0;
for (cancion of cantidadCanciones) {
    cantidadTotalCanciones += (cancion.songs).length;
}

let promedioDuracion = 0;
for (duracion of cantidadCanciones) {
    promedioDuracion += (cancion.duration) / cantidadTotalCanciones;
}
///// RESULTADO
